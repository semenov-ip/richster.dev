<?php if (!defined('BASEPATH')) exit('No direct script access allowed');
    ...
    $CI =& get_instance();
    $CI->load->model('select_models');
    $userSmsAccount = $CI->select_models->select_one_row_where_column(array('sms_id' => 1),'sms_account');

    // SMSC.RU API (smsc.ru) версия 2.7 (07.06.2012)
    define("SMSC_LOGIN", $userSmsAccount->SMSC_LOGIN);          // логин клиента
    define("SMSC_PASSWORD", $userSmsAccount->SMSC_PASSWORD);    // пароль или MD5-хеш пароля в нижнем регистре
    ...
    function send_sms($phones, $message, $translit = 0, $time = 0, $id = 0, $format = 0, $sender = 'Richster', $query = ""){
